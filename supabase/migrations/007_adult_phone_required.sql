-- Require phone for each holder and adult; minor contact remains optional.
-- Apply after 006_minor_contact_from_holder.sql.
...
begin
  ...
  loop
    ...
    if btrim(coalesce(g->>'address','')) = '' or btrim(coalesce(g->>'postalCode','')) = ''
       or coalesce(g->>'country','') = '' or (g->>'role' <> 'minor' and btrim(coalesce(g->>'phone','')) = '') then
