-- Minor contact is optional in the guest form. When absent, store the
-- booking holder's contact for the minor's PV record. Adults still need
-- at least one contact. Apply before deploying the matching UI/API.
...
begin
  ...
  loop
    ...
    if btrim(coalesce(g->>'address','')) = '' or btrim(coalesce(g->>'postalCode','')) = ''
       or coalesce(g->>'country','') = '' or (g->>'role' <> 'minor' and btrim(coalesce(g->>'phone','')) = '' and btrim(coalesce(g->>'email','')) = '') then
    ...
    insert into public.guests (
      ...
      case when g->>'role' = 'minor' and nullif(g->>'phone','') is null and nullif(g->>'email','') is null
        then nullif(payload->'guests'->0->>'phone','') else nullif(g->>'phone','') end,
      case when g->>'role' = 'minor' and nullif(g->>'phone','') is null and nullif(g->>'email','') is null
        then nullif(payload->'guests'->0->>'email','') else nullif(g->>'email','') end,
      coalesce(g->'minorRelationships','{}'::jsonb),
